...
class car:                      # parent class
    ...
    def info(self):
        print("wheels is",self.wheels," type is ",self.type," capacity is ",self.capacity,"  : ",self.company)

class ford(car):              # child class decleration

    def __init__(self,color,modal,year,comnany):
        super().__init__(comnany)                        # super method is used to call parent __init__() func in child class
        self.color = color
        self.year = year
        self.modal = modal
...
child2.display();    # output will "mother" because order is change , now class mother invoke first
