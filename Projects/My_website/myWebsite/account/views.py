...
import uuid
from .models import Profile, UserContent
...
class RegisterAPI(APIView):
    def post(self, request):
        ...
        if not serializers.is_valid():
            return Response({"status": False, "messages": serializers.errors})
        else:
            serializers.save()
...
class GetBookmarkList(APIView):
    def get(self, request):
        try:
            data = UserContent.objects.get(user=request.user.id)
        except:
            return Response({"message": "no bookmark found"})
        serializers = GetBookmarkListSerializer(data)
...
class AddBookmark(APIView):
    def post(self, request, id):
        ...
        try:
            obj = UserContent.objects.get(user=request.user.id)
        except:
            obj = UserContent(user=request.user)
            obj.save()
        obj.bookmarks.add(blog)
        obj.save()
        return Response({"message": "blog is added in bookmark list"})
